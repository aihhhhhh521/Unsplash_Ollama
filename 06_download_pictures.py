...
from config import CLASSIFIED_FILE, WORK_DIR
...
RAW_IMG_ROOT = IMG_ROOT / "raw"
META_ROOT = OUT_ROOT / "metadata"

RAW_IMG_ROOT.mkdir(parents=True, exist_ok=True)
...
DOWNLOAD_RESULTS_JSONL = META_ROOT / "download_results_raw.jsonl"
...
def build_raw_relpath(photo_id: str, ext: str) -> Path:
    pid = safe_str(photo_id)
    shard1 = (pid[:2] or "xx").lower()
    shard2 = (pid[2:4] or "xx").lower()
    return Path("raw") / shard1 / shard2 / f"{pid}{ext}"
    ...
    for attempt in range(1, RETRY + 1):
        try:
            with requests.get(url, stream=True, timeout=REQUEST_TIMEOUT, headers=headers) as resp:
                ...
                relpath = build_raw_relpath(photo_id, ext)
...
def main() -> None:
    ...
    todo_rows = []
    cached_records = []
    for row in df[["photo_id", "photo_image_url"]].to_dict("records"):
    ...
    print(f"[OK] 图片目录（raw）: {RAW_IMG_ROOT}")
